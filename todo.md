...
| Compact Library cards, live site filters and site families | Codex `/root` | in progress | `src/domain/library.ts`, `src/entrypoints/library/App.tsx`, `src/entrypoints/library/components/PageCard.tsx`, `src/entrypoints/library/style.css`, `tests/library.test.ts`, `tests/libraryCard.test.ts` (new), `e2e/library.spec.ts`, `README.md`, `docs/ACCEPTANCE-TEST-PLAN.md`, `docs/HANDOFF-STORE-SUBMISSION.md`, `package.json`, `scripts/store-fields.mjs`, `docs/STORE-SUBMISSION.md`, `todo.md`, release assets | By-page cards show at most five matching annotations by default, expose overflow through an ellipsis control, and pack variable-height cards without grid-row gaps. Site chips will follow the current live/deleted collection, clear an invalid selected chip after the last matching annotation is removed, and merge host variants into readable site families (for example `www.nature.com` → `nature`, `github.com` and `*.github.io` → `github`). |
