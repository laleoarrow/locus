...
**Versioning:** `v0.7.0` is the latest published release (Timeline journey,
atomic whole-library colour replacement and native-safe image gestures).
`package.json` sits at `0.7.0`.

## Messages

- 2026-07-31 — **Codex `/root` released v0.7.0 and completed the local rollout.** GitHub Latest is <https://github.com/laleoarrow/locus/releases/tag/v0.7.0>; release commit `37a9374` contains the approved bulk-colour flow, Timeline journey, native-safe image drag gesture and backup/sync conflict fixes. Final gates passed: typecheck, 128 unit, 39 E2E, Chrome + Edge production builds. The independently downloaded keyed Chrome and Edge sideload assets both have SHA-256 `5c8afe1c8c654c64cc256b7656eb8bf435a722fd67bb595f7d40fc50e09bfe51`; the keyless Edge store/Desktop package has SHA-256 `909b832f9886e72976b5a4b717c261c6253e53ec5b94da4c379281c72329e641`. Edge was Reloaded in place to v0.7.0 at fixed ID `dfmekdplmdbjbefchginloeehjonjlan`; its live Library retained all 22 current annotations. Reproducible build directories, local ZIP copies, test output and independent-download verification files were moved to Trash; `.output` retains only the active `edge-mv3`. Partner Center remains blocked at Email Verification step 2/4, so no store Submit was attempted.
- 2026-07-31 — **Codex `/root` completed bulk color replacement for UI review.** The compact `Batch → Replace annotation colours…` flow performs one atomic whole-library live-row update; Detached annotations are included, Deleted/tombstone rows are excluded, stale preview counts abort cleanly, and existing filters remain intact with an active source-color filter migrated to the target. The old-background `null.updated` failure was handled explicitly and the user's unpacked Edge extension was reloaded in place; all 19 live annotations remain. Final gates passed: typecheck, 124 unit, 36 E2E, Chrome + Edge production builds. No version bump, commit, push or release was made.
